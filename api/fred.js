    ...
    const results = await Promise.allSettled(
      SERIES.map(s =>
        fetch(`${BASE}?series_id=${s.id}&api_key=${FRED_KEY}&sort_order=desc&limit=1&file_type=json`)
          .then(r => r.json())
          .then(d => d.observations && d.observations[0] ? d.observations[0] : null)
      )
    );

    const data = SERIES.map((s, i) => {
      const obs = results[i].status === 'fulfilled' ? results[i].value : null;
      if (!obs || obs.value === '.') return { label: s.label, suffix: s.suffix, desc: s.desc, display: null, date: null };
      const raw = parseFloat(obs.value);
      const display = s.div
        ? (raw / s.div).toFixed(1) + s.suffix
        : raw.toFixed(2) + s.suffix;
      return { label: s.label, desc: s.desc, display, date: obs.date };
    });
